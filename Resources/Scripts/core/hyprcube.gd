...
func _ready() -> void:
	...
	if world_node == null:
		push_error("Hyprcube: Could not find 'Level' node. Please assign it to the 'world_node' export var in the Inspector.")
		set_process(false) # Disable script if not set up correctly
